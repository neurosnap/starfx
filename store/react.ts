...
export function useStore<S extends AnyState = AnyState>(): FxStore<S> {
  const store: any = useBaseStore();
  return store as FxStore<S>;
}

export function useSchema<S extends AnyState = AnyState>() {
